...
#define TAG "ti.immersivemode"

using namespace v8;

static Persistent<Object> bindingCache;

static void Immersivemode_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Immersivemode.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::ImmersivemodeBindings::lookupGeneratedInit(
...
static void Immersivemode_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Immersivemode_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Immersivemode_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::ImmersivemodeBindings::lookupGeneratedInit(*binding, bindingLength);
...
Java_ti_immersivemode_ImmersivemodeBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.immersivemode", &ImmersivemodeBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::ImmersivemodeBindings::lookupGeneratedInit));
}
